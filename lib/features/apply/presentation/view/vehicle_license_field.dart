...
class VehicleLicenseField extends StatefulWidget {
  const VehicleLicenseField({super.key});

  @override
  State<VehicleLicenseField> createState() => _VehicleLicenseFieldState();
}

class _VehicleLicenseFieldState extends State<VehicleLicenseField> {
  ...
  Future<void> _handleImageSelection(ImageSource source) async {
    ...
    try {
      ...
      if (image != null && mounted) {
        setState(() => viewModel.vehicleLicenseImage = File(image.path));

        viewModel.doIntent(ExtractDataFromVehicleLicenseAction());
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<ApplyCubit, ApplyState>(
      ...
      listener: (context, state) {
        if (state.extractDataFromVehicleLicenseState is BaseErrorState) {
          final rawError =
              (state.extractDataFromVehicleLicenseState as BaseErrorState)
                  .errorMessage;
          final userMessage = rawError.replaceFirst('Exception:', '').trim();
          AppDialogs.showFailureDialog(
            context,
            message: LocaleKeys.InvalidVehicleLicense.tr(),
        ...
        child: AbsorbPointer(
          ...
          child: TextFormField(
            decoration: InputDecoration(
              ...
              hintText:
                  viewModel.vehicleLicenseImage == null
                      ...
                      : Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: DecorationImage(
                            image: FileImage(viewModel.vehicleLicenseImage!),
                            fit: BoxFit.cover,
                          ),
                        ),
                      ),
